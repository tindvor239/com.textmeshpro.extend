...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        [SerializeField] private bool hideButtonOnReachLimit = false;
        ...
        private void Setup()
        {
            ...
            Panels = Content.GetComponentsInChildren<RectTransform>(false).Where(go => go != Content).ToArray();
        ...
        private void SelectPanel()
        {
            ...
            if (releaseSpeed <= minimumSwipeSpeed)
            {
            ...
            switch (snapTarget)
            {
                case SnapTarget.Nearest:
                    GoToPanel(nearestPanel);
                    return;
                case SnapTarget.Previous:
        ...
        private void SnapToPanel()
        {
            ...
            Vector2 contentOffset = -Panels[CenteredPanel].anchoredPosition;
            switch(movementAxis)
            {
                case MovementAxis.Horizontal:
                    contentOffset.y = 0;
            ...
            Vector2 targetPosition = contentOffset + offset;
        ...
        public void GoToPanel(int panelNumber, bool canCallBack = true)
        {
            ...
            if (!hideButtonOnReachLimit)
            {
                return;
            }

            previousButton.interactable = panelNumber > 0;
            nextButton.interactable = panelNumber < NumberOfPanels - 1;
        }
        ...
        public void Remove(int index)
        {
            ...
            if (CenteredPanel == index)
            {
                startingPanel = index == NumberOfPanels ? CenteredPanel - 1 : CenteredPanel;
            }
